...
def create_bipartite_network(cluster_dict):
    ...
    for k, v in cluster_dict.items():
        if len(v) > 2:
            B.add_node(k, bipartite=0)  # Node type U
            for node in v:
                B.add_node(node, bipartite=1)  # Node type V
                B.add_edge(k, node)
    return B
...
def write_edgelist(B, filename):
    with open(filename, 'w') as f:
        for u, v, data in B.edges(data=True):
            if B.nodes[u]['bipartite'] == 0:  # If u is of type U
                f.write(f"{u} {v} {data}\n")
            else:  # If u is of type V
                f.write(f"{v} {u} {data}\n")
...
def run_network_analysis(anchor_file, final_output_path, base_output_name):
    ...
    # Assert if the graph is bipartite
